...
async def async_get_config_entry_diagnostics(
    ...
    try:
        ...
        coordinator: PoolSyncDataUpdateCoordinator | None = hass.data.get(
            DOMAIN, {}
        ).get(entry.entry_id)
        ...
        if coordinator:
            coordinator_status_info = {
                ...
                "last_update_success": getattr(
                    coordinator, "last_update_success", "N/A"
                ),
                "coordinator_name": getattr(coordinator, "name", "N/A"),
                "mac_address_from_coordinator": getattr(
                    coordinator, "mac_address", "N/A"
                ),
                "ip_address_from_coordinator": "N/A",
                "update_interval_seconds": None,
            }
            if hasattr(coordinator, "api_client") and hasattr(
                coordinator.api_client, "_ip_address"
            ):
                coordinator_status_info["ip_address_from_coordinator"] = (
                    coordinator.api_client._ip_address
                )
            if hasattr(coordinator, "update_interval") and coordinator.update_interval:
                coordinator_status_info["update_interval_seconds"] = (
            ...
            diagnostics_data["coordinator_data_payload"] = (
                coordinator.data if coordinator.data is not None else "Data is None"
            )
            ...
            try:
                ...
                if mac_address_for_lookup:
                    device = device_registry.async_get_device(
                        identifiers={(DOMAIN, mac_address_for_lookup)}
                    )
                    if device:
                        diagnostics_data["device_registry_info"] = {
                            "id": device.id,
                            "identifiers": list(
                                list(identifier) for identifier in device.identifiers
                            ),
                            "connections": list(
                                list(connection) for connection in device.connections
                            ),
                            ...
                            "disabled_by_device_reg": str(device.disabled_by)
                            if device.disabled_by
                            else None,
                        }
                    else:
                        diagnostics_data["device_registry_info"] = (
                            f"Device with MAC {mac_address_for_lookup} not found in registry."
                        )
                else:
                    diagnostics_data["device_registry_info"] = (
                        "MAC address not available from coordinator to look up device."
                    )
            except Exception as e_dev_reg:
                _LOGGER.error(
                    "Error gathering device registry info for diagnostics: %s",
                    e_dev_reg,
                )
                diagnostics_data["device_registry_info"] = f"Error: {str(e_dev_reg)}"
                current_error = diagnostics_data.get("error_in_diagnostics", "") or ""
                diagnostics_data["error_in_diagnostics"] = (
                    current_error + f"DeviceReg Error: {traceback.format_exc()}; "
                )
        else:
            _LOGGER.warning(
                "Diagnostics: Coordinator not found in hass.data for entry ID %s.",
                entry.entry_id,
            )

    except Exception as e_diag:
        _LOGGER.exception(
            "Unexpected error while gathering diagnostics for entry ID %s: %s",
            entry.entry_id,
            e_diag,
        )
        current_error = diagnostics_data.get("error_in_diagnostics", "") or ""
        diagnostics_data["error_in_diagnostics"] = (
            current_error + f"Overall Diagnostics Error: {traceback.format_exc()}"
        )

    _LOGGER.debug(
        "Finished gathering diagnostics for entry ID %s. Error in diagnostics: %s",
        entry.entry_id,
        diagnostics_data["error_in_diagnostics"],
    )
